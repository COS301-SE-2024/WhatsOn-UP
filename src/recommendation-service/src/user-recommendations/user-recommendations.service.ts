...
export class UserRecommendationsService {
  ...
  async getRecommendedEvents(userID: string): Promise<any> {
    ...
    if (catErr) {
      throw new Error(`Error fetching user category preferences: ${catErr.message}`);
    }
    //////////////////
    ...
    const predictedRatings = events.map(event => {
      ...
      const interactionPreferenceValue = preferenceMap.get(category).interaction || 0;
      const surveyPreferenceValue = preferenceMap.get(category).survey || 0;

      const binaryDigit = 1; // always 1
      const rating = (interactionPreferenceValue + surveyPreferenceValue) * binaryDigit;

      // Log the computed rating for debugging
