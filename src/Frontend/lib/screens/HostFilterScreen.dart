...
class HostFilterScreen extends StatefulWidget {
  ...
  HostFilterScreen({required this.hostId});
...
class _HostFilterScreenState extends State<HostFilterScreen> with SingleTickerProviderStateMixin {
  ...
  void _filterEvents() async {
    ...
    try {
      ...
      // Ensure 'hosts' is a list of 'Host' objects and use the correct property.
      List<Event> hostEvents = events.where((event) => event.hosts.any((host) => host.userId == widget.hostId)).toList();
      Navigator.of(context).push(MaterialPageRoute(
        builder: (context) => FilteredResultScreen(filteredEvents: hostEvents),
      ));
    } catch (error) {
