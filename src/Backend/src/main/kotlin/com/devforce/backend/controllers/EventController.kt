...
class EventController {
    ...
    fun getEvents(): ResponseEntity<Any> {
        return try {
            ...
            ResponseEntity.ok(ResponseDTO("success", System.currentTimeMillis(), events))
        } catch (e: Exception) {
            ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(ResponseDTO("error", System.currentTimeMillis(), "An error occurred while retrieving events"))
        }
    }
    ...
    fun addEvent(@RequestBody eventData: Map<String, String>): ResponseEntity<Any> {
        ...
        if (name.isNullOrEmpty() || description.isNullOrEmpty() || startTime.isNullOrEmpty() || endTime.isNullOrEmpty()) {
            return ResponseEntity.badRequest().body(ResponseDTO("error", System.currentTimeMillis(), "Name, description, start time, and end time are required"))
        }
        ...
        return try {
            ...
            ResponseEntity.ok(ResponseDTO("success", System.currentTimeMillis(), mapOf("event" to savedEvent)))
        } catch (e: IllegalArgumentException) {
            ResponseEntity.badRequest().body(ResponseDTO("error", System.currentTimeMillis(), "Invalid date format"))
        }
    }


    @PostMapping("/delete_event")
    fun deleteEvent(@RequestBody eventData: Map<String, String>): ResponseEntity<ResponseDTO> {
        val eventId = eventData["eventId"]
        if (eventId.isNullOrEmpty()) {
            return ResponseEntity.badRequest().body(ResponseDTO("error", System.currentTimeMillis(),  "Event ID is required"))
        }
        ...
        return try {
            ...
            if (eventRepository.existsById(uuid)) {
                ...
                ResponseEntity.ok(ResponseDTO("success", System.currentTimeMillis(), "Event deleted successfully"))
            } else {
                ResponseEntity.status(HttpStatus.NOT_FOUND).body(ResponseDTO("error", System.currentTimeMillis(), "Event not found"))
            }
        } catch (e: IllegalArgumentException) {
            ResponseEntity.badRequest().body(ResponseDTO("error", System.currentTimeMillis(), "Invalid Event ID format"))
        }
    }
